...
If you have a GitHub account, you can use it to fork this repository and then request a run under the Actions tab.  This is set to produce MWR, Lilypond, PDF, MIDI and MP3 output in the zip-downloadable Artifact of the run, although it might not be your preferred soundfont (sorry but there are licensing considerations in the Actions setup).

The run length can vary from minutes to hours depending on the random number seed.  The default for GitHub Actions is a seed that completes in minutes, but you can try others: if the run takes too long then I suggest you interrupt it and try a different seed.
